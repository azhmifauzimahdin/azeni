...
import { Invitation, ThemeBackground } from "@/types";
...
const Luxury2BasicPage: React.FC<Invitation> = (initialInvitation) => {
  ...
  const images = useMemo(
    () =>
      invitation.themeBackgrounds?.map((g: ThemeBackground) => g.image) || [],
    [invitation.themeBackgrounds]
  );
  ...
  return (
    <div className="flex justify-end min-h-screen bg-gray-100 text-sm font-raleway">
      <LeftSidebar
        imageSrc={
          invitation.themeBackgrounds?.find((bg) => bg.image.includes("002"))
            ?.image ?? ""
        }
      >
                  ...
                  {invitation.stories.map((story, index) => (
                    <div key={index} className="mb-10" data-aos="fade-up">
                      {story.image && (
                        <div
                          className={cn(
                            "w-full bg-white p-3 pb-10 mb-3 mt-10",
...
export default Luxury2BasicPage;
